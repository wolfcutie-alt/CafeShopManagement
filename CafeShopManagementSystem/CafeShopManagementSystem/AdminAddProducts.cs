...
namespace CafeShopManagementSystem
{
    public partial class AdminAddProducts : UserControl
    {
            ...
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        // CHEKING IF THE PRODUCT ID IS EXISTING ALREADY
                        ...
                        using (SqlCommand selectPID = new SqlCommand(selectProdID, connect))
                        {
                            ...
                            {
                                string insertData = "INSERT INTO products (prod_id, prod_name, prod_type, prod_stock, prod_price, prod_status, prod_image, date_insert) VALUES(@prodID, @prodName, @prodType, @prodStock, @prodPrice, @prodStatus, @prodImage, @dateInsert)";

                                DateTime today = DateTime.Today;

                                string path = Path.Combine(@"D:\Project\CafeShopManagementSystem\CafeShopManagementSystem\Product_Directory\" + adminAddProducts_id.Text.Trim() + ".jpg");
        ...
        private void adminAddProducts_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Are you sure you want to UPDATE PRODUCT ID: " + adminAddProducts_id.Text.Trim() + "?", "Comfirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            using (SqlCommand updateD = new SqlCommand(updateData, connect))
                            {
                                ...
                                updateD.Parameters.AddWithValue("@prodStatus", adminAddProducts_price.Text.Trim());
        ...
        private void adminAddProducts_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Are you sure you want to DELETE PRODUCT ID: " + adminAddProducts_id.Text.Trim() + "?", "Comfirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
