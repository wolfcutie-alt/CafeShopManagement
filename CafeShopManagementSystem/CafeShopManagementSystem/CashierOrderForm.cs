...
namespace CafeShopManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\natha\Documents\cafe.mdf;Integrated Security=True;Connect Timeout=30"))
            {
                connect.Open();
        ...
        private void cashierOrderForm_payBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Something wnet wrong.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (MessageBox.Show("Are you sure for paying?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            string insertData = "INSERT INTO customers (Customer_id, total_price, amount, change, date) VALUES(@custID, @totalprice, @amount, @change, @date)";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "Nathan's Cafe Shop";
            y = (margin + count * headerFont.GetHeight(e.Graphics) + headerMargin);
            e.Graphics.DrawString(headerText, headerFont, Brushes.Black, e.MarginBounds.Left + (cashierOrderForm_orderTable.Columns.Count / 2) * colWidth, y, alignCenter);
